...
export class Sidebar {
    ...
    initScene() {
...

        {
            ...
            const exportTxt = i18n.t("gismatrix.export");
            ...
            const noMeasurementsToExportTxt = i18n.t("gismatrix.noMeasurementsToExport");
            ...
            elDownloadJSON.click((event) => {
                ...
                if (measurements.length > 0) {
                    ...
                    this.viewer.postError(noMeasurementsToExportTxt);
                    event.preventDefault();
                }
            });
        ...
        const pointCloudsTxt = i18n.t("gismatrix.pointClouds");
        const measurementTxt = i18n.t("gismatrix.measurement");
        const annotationsTxt = i18n.t("gismatrix.annotations");
        const otherTxt = i18n.t("gismatrix.other");
        const vectorsTxt = i18n.t("gismatrix.vectors");
        const imagesTxt = i18n.t("gismatrix.images");


        let pcID = tree.jstree('create_node', "#", { "text": "<b>" + pointCloudsTxt + "</b>", "id": "pointclouds" }, "last", false, false);
        let measurementID = tree.jstree('create_node', "#", { "text": "<b>" + measurementTxt + "</b>", "id": "measurements" }, "last", false, false);
        let annotationsID = tree.jstree('create_node', "#", { "text": "<b>" + annotationsTxt + "</b>", "id": "annotations" }, "last", false, false);
        let otherID = tree.jstree('create_node', "#", { "text": "<b>" + otherTxt + "</b>", "id": "other" }, "last", false, false);
        let vectorsID = tree.jstree('create_node', "#", { "text": "<b>" + vectorsTxt + "</b>", "id": "vectors" }, "last", false, false);
        let imagesID = tree.jstree('create_node', "#", { "text": "<b>" + imagesTxt + "</b>", "id": "images" }, "last", false, false);
        ...
        const cameraTxt = i18n.t("gismatrix.camera");

        {
            createNode(otherID, cameraTxt, null, new THREE.Camera());
        }
    ...
    initClippingTool() {
        ...
        const clipTaskTxt = i18n.t("gismatrix.clipTask");
        const clipMethodTxt = i18n.t("gismatrix.clipMethod");

        {
            let elClipTask = $("#cliptask_options");
            elClipTask.selectgroup({ title: clipTaskTxt });
...

        {
            ...
            elClipMethod.selectgroup({ title: clipMethodTxt });
    ...
    initNavigation() {
        ...
        const cameraOrthographicTxt = i18n.t("gismatrix.cOrthographic");

        let elCameraProjection = $(`
			<selectgroup id="camera_projection_options">
				<option id="camera_projection_options_perspective" value="PERSPECTIVE">${cameraPerspectiveTxt}</option>
				<option id="camera_projection_options_orthigraphic" value="ORTHOGRAPHIC">${cameraOrthographicTxt}</option>
			</selectgroup>
		`);
        elNavigation.append(elCameraProjection);
        elCameraProjection.selectgroup({ title: cameraProjectionTxt });
